...
**Document Object Model (DOM)** - A JavaScript interface that represents a web-page's structure and lets you interact with and change its elements dynamically.
...
**Function** - A reusable piece of code that does task(s) and can be called to execute those tasks when needed.

**Event Listener** - A programming object that listens for specific events to come before executing a function.
...
- Place a double forward slash "//" at the beginning for single-line comments
- Place a forward slash and an asterisk "/*" and end it with "*/" for multi-line comments

### Variables

**Variable** - A named location in the computer's memory where data can be stored and retrieved during the execution of a program.
...
### Function Types and Concepts

**Function Call** - The act of executing a function to perform its defined tasks or operations.

**Arguments** - Values passed when functions are called; they are the values provided within the parentheses.
...
**Arrow Function** - A different approach to defining a function that does not contain a name or identifier, defined by including the parentheses, the arrow "=>", and then a block of code.
...
1. **Single Parameter**: `array.map( x => x * 2 );`
2. **Multiple Parameters**: `array.map( ( num1, num2 ) => num1 * num2 );`
3. **Block Body**: `array.map( ( num1, num2 ) => { let result = num1 * num2; return result; }`
...
**Index Indicator** - Special syntax appended to the end of a collection object that includes a set of square brackets and a whole number to indicate the position of the element being accessed.
...
**IF/IF-ELSE** - A common programming structure that controls the flow of execution depending on the evaluation of a true/false condition statement.
...
**Programming Handle** - An object or variable that represents another object in memory.

## AJAX and Web Communications

### AJAX Concepts

**AJAX (Asynchronous JavaScript And XML)** - A technique which uses the asynchronous architecture of JavaScript to:
...
**Internet Information Service (IIS)** - Software that hosts files and resources online and responds to HTTP requests.
...
**PHP (Hypertext Preprocessor)** - A powerful, general-purpose, open-source scripting language.
...
**Echo** - The PHP command that specifies what text content will be left in place of a PHP container of code. Returns nothing and is slightly faster than print.

**Print** - The PHP command that specifies what text content will be left in place of a PHP container of code. Returns a value of 1 and is slightly slower than echo.
...
## Compatibility and Development Practices
...
**Authentication Token** - A digital file that serves as proof of authentication and can be used to access one or more systems without having to reauthenticate each time.
